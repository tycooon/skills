...
- If what you pushed changes anything the PR's title or description says — what it does, its scope, the numbers it quotes, the tests run — edit them to match right after the push (GitHub: `gh pr edit <pr> --title '…' --body-file <file>`; GitLab: `glab mr update <mr> --title '…' --description '…'`). The reviewer judges the PR by that text, and a stale one has them approving something the code no longer does.
