...
**A scheduled resume can be lost, and nothing will tell you.** The wake-up is the only thing that brings the watch back, so one that never fires ends the watch as surely as a stop — with no error, no final report, and a last turn that looks finished. It happens: in one sweep ten resumes in a single afternoon never fired. Nine were masked by other events that happened to wake the session near the due time, and the tenth left the watch idle for 45 minutes until the user asked why it had stopped; in that time six PRs were opened and one was opened and merged with no review at all. So when the watch starts, arm a backup heartbeat next to the wake-up: a recurring, session-only job that re-enters this skill on a fixed clock — in Claude Code, `CronCreate` on off-minutes every 15 minutes (e.g. `7,22,37,52 * * * *`). Its prompt reads the state file first and does nothing unless the next round is overdue — the due time you recorded has passed by a few minutes; only then does it run the round exactly as the wake-up would, arming the next wake-up included. It never paces the watch — the table above still does — it only caps what a lost resume can cost. Record its id in the state file and delete it (`CronDelete`) in the same turn as any stop: `ScheduleWakeup`'s `stop` does not touch it, and an orphaned heartbeat keeps re-entering a finished watch until it expires.
